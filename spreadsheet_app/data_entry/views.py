    ...
    if request.method == "POST":
        ...
        if employee_formset.is_valid():
            for form in employee_formset:
                if form.cleaned_data.get('break_time') is not None:
                    form.cleaned_data['break_time'] = str(form.cleaned_data['break_time']).replace(',', '.')
                form.save()
            # return redirect('views_page')  # Prevents duplicate submissions
