...
@app.callback(
    [Output("weekly-plot", "figure"), 
     ...
     Output("filtered-data", "data")],  # Store the filtered data
...
def update_graphs(start_date, end_date):
    ...
    weekly_data = filtered_df.groupby("week", as_index=False)["difference"].sum().reset_index(drop=True)
    monthly_data = filtered_df.groupby("month", as_index=False)["difference"].sum().reset_index(drop=True)
    ...
    # Weekly Figure
    weekly_fig = px.bar(
        weekly_data, x="week", y="Total Hours",
        title="Work Hours per Week",
        text=weekly_data["Total Hours"].astype(int)
    )
    weekly_fig.update_xaxes(type="category")
    weekly_fig.update_traces(texttemplate="%{text} Stunden", hovertemplate="Week: %{x} <br>Total Hours: %{y:.0f} Stunden")
    weekly_fig.update_yaxes(range=[0, weekly_data["Total Hours"].max() + 5])

    # Monthly Figure
    monthly_fig = px.bar(
        monthly_data, x="month", y="Total Hours",
        title="Work Hours per Month",
        text=monthly_data["Total Hours"].astype(int)
    )
    monthly_fig.update_xaxes(type="category")
    monthly_fig.update_traces(texttemplate="%{text} Stunden", hovertemplate="Month: %{x} <br>Total Hours: %{y:.0f} Stunden")
