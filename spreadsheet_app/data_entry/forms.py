...
class EmployeeForm(forms.ModelForm):
    work_start = forms.TimeField(widget=forms.TimeInput(format='%H:%M', attrs={'type': 'time', 'step': '60'}), initial='06:00', localize=True, validators=[validate_time,])
    work_end = forms.TimeField(widget=forms.TimeInput(format='%H:%M', attrs={'type': 'time', 'step': '60'}), initial='14:30', localize=True, validators=[validate_time,])
    break_time = CommaFloatField(decimal_places=1, localize=True, initial="0.5", required=False,  widget=forms.NumberInput(attrs={
            'style': 'width: 50px; text-align: center;',
            'step': '0.1',  # Allows decimals like 4.5, 3.0, etc.
            'min': '0', 
        }))
    attribut = forms.CharField(required=False, widget=forms.TextInput(attrs={'style':'width:80px'}))
...
class WorkCategoryForm(forms.ModelForm):
    ...
    cleaning = CommaFloatField(decimal_places=1, localize=True, initial=0,
                                widget=forms.NumberInput(attrs={
        'style': 'width: 100px; text-align: center;',
        'step': '0.1',  # Allows decimals like 4.5, 3.0, etc.
        'min': '0', }))
    maintenance = CommaFloatField(decimal_places=1, localize=True, initial=0, 
                                  widget=forms.NumberInput(attrs={
        'style': 'width: 100px; text-align: center;',
        'step': '0.1',  # Allows decimals like 4.5, 3.0, etc.
        'min': '0', }))
    interruption = CommaFloatField(decimal_places=1, localize=True,  initial=0,
                                    widget=forms.NumberInput(attrs={
        'style': 'width: 100px; text-align: center;',
        'step': '0.1',  # Allows decimals like 4.5, 3.0, etc.
        'min': '0', }))
...
class WorkHoursForm(forms.ModelForm):
    ...
    start_time = forms.TimeField(widget=forms.TimeInput(format='%H:%M', attrs={'type': 'time'}), initial='00:00', validators=[validate_time,], 
                                 error_messages={validate_time: 'Keine gültige Zeit!'}, input_formats=valid_time_formats, localize=True)
    end_time = forms.TimeField(widget=forms.TimeInput(format='%H:%M', attrs={'type': 'time'}), initial='00:00', validators=[validate_time,], localize=True)
