...
from .utils import (
    get_kuebel_data,
    plot_tages_werte_nach_aktivitaet,
    plot_tages_werte_aktivitaet_anzahl,
...
def grouped_dashboard(request):
    ...
    end_date = (today.replace(day=28) + timedelta(days=4)).replace(day=1) - timedelta(
        days=1
    )
    ...
    if request.GET:
        ...
        start = make_aware(
            datetime.combine(start_date, time.min), timezone=timezone("Europe/Vienna")
        )
        end = make_aware(
            datetime.combine(end_date, time.max), timezone=timezone("Europe/Vienna")
        )
    ...
    if start and end:
        ...
        if "download" in request.GET:
            ...
            response["Content-Disposition"] = (
                'attachment; filename="Kuebeleingaben_Details.xlsx"'
            )
            return response
    ...
    return render(
        request,
        "him2_dboard_kuebelwaschplatz/grouped_dashboard.html",
        {
