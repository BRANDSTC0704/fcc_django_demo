...
class SchichtEingabeMitarbeiter(models.Model):
    ...
    mitarbeiter_1 = models.ForeignKey(
        Mitarbeiter, on_delete=models.PROTECT, related_name="+"
    )
    mitarbeiter_2 = models.ForeignKey(
        Mitarbeiter, on_delete=models.PROTECT, related_name="+"
    )
    ...
    def clean(self):
        if self.mitarbeiter_1 == self.mitarbeiter_2:
...
class Produktion(models.Model):
    ...
    mitarbeiter = models.ForeignKey(
        Mitarbeiter, related_name="Produktionsmitarbeiter", on_delete=models.PROTECT
    )  # soll immer gespeichert bleiben
    schicht = models.ForeignKey(
        Schicht, on_delete=models.PROTECT
    )  # kann nicht gelöscht werden, wenn Daten da
    abh_prod = models.ForeignKey(
        AbhProdTyp, on_delete=models.PROTECT
    )  # kann nicht gelöscht werden, wenn Daten da
    ballentyp = models.ForeignKey(
        PresseBallenTyp, on_delete=models.PROTECT
    )  # kann nicht gelöscht werden, wenn Daten da
