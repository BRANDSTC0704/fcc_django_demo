...
class KuebelEintragFormTest(TestCase):
    ...
    def test_form_valid_data(self):
        form = KuebelEintragForm(
            data={"log": self.session.id, "kuebel_art": self.art.id, "sonstiges_h": 2.5}
        )
...
class KuebelSessionFormTest(TestCase):
    ...
    def setUp(self):
        ...
        self.session = KuebelSession.objects.create(
            user=self.user, mitarbeiter=self.mitarbeiter, comments=self.comments
        )

    def test_form_valid_data(self):
        form = KuebelSessionForm(
            data={"mitarbeiter": self.mitarbeiter, "comments": self.comments}
        )
        self.assertTrue(form.is_valid())
...
class KuebelSessionViewTest(TestCase):
    ...
    def test_create_kuebel_session_invalid_created_at(self):
        ...
        if response.status_code == 200:
            ...
            now = timezone.now()
            self.assertTrue(
                now - log.created_at < timedelta(seconds=2),
