...
class KuebelSessionAdminForm(forms.ModelForm):
    ...
    created_at_time = forms.TimeField(
        label="erstellt am (Uhrzeit [hh:mm])", widget=forms.TimeInput(format="%H:%M")
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["created_at_date"].initial = created_at.strftime(
            "%Y-%m-%d"
        )  # created_at.date()
        self.fields["created_at_time"].initial = created_at.time().replace(
